...
const inputSchema = z.object({
  ...
  query: z.string().trim().min(1).max(200).optional().describe('Free-text match on quotation number or item name'),
});

export const searchQuotationsTool: AiToolDefinition<z.infer<typeof inputSchema>> = {
  name: 'search_quotations',
  description: 'Search quotations (price offers) by customer, status, or free text (quotation number / item name). Returns a short summary list — call get_quotation for full detail on one.',
  ...
  inputJsonSchema: {
    ...
    properties: {
      ...
      query: { type: 'string', description: 'Free-text match on quotation number or item name' },
    },
  ...
  async execute(input, ctx) {
    ...
    let dtos = quotations.map((q) => mapQuotationToDto(q, canSeeInternal));
